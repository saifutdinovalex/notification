...
var Notifications = (function(options) {
    ...
    this.renderRow = function (object) {
        var keywords = ['id', 'title', 'description', 'url', 'type', 'key'];
        ...
        html = '<div class="notification notification-{type} ' +
            ...
            '" data-route="{url}" data-id="{id}" data-key="{key}">' +
                html +
                '</div>';
        ...
        ret.find('.notification-seen').click(function() {
            ...
            var dropdown = $(this).parents('.notification-dropdown');
            // Update all counters
            
                for (var i = 0; i < self.opts.counters.length; i++) {
                    $(dropdown).find(self.opts.counters[i]).text(parseInt($(dropdown).find(self.opts.counters[i]).html())-1);
                }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (difference.length > 0) {
                    ...
                    console.log(self.opts.listSelector);
                ...
                for (var i = 0; i < data.counts.length; i++) {
                    for (var j = 0; j < self.opts.counters.length; j++) {
                        $('.'+data.counts[i].class_name).find(self.opts.counters[j]).text(data.counts[i].count);
                    }
                }
